...
# 运行此文件需要，在终端输入 streamlit run ui/streamlit_app.py
# 启动之前, 后端服务器要启动，比如本项目使用的是 fastapi的 "http://127.0.0.1:8002"
...
with col1:
    ...
    if st.button("上传并入库", use_container_width=True, type="primary") and uploaded:
        ...
        r = requests.post(f"{API_BASE}/ingest", files=files, data=data, timeout=120)                # 请求 /ingest
        if r.ok:
    ...
    if st.button("全量重建（清库+重建）", use_container_width=True):
        r = requests.post(f"{API_BASE}/reindex", data={"visibility_default": "public"}, timeout=300)        # 请求 /reindex
        if r.ok:
...
with col2:
    ...
    if st.button("发送", use_container_width=True) and q.strip():
        ...
        r = requests.post(f"{API_BASE}/chat", json=payload, timeout=120)                            # 请求 /chat
        if r.ok:
            st.markdown("**回答：**")
